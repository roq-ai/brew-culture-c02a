...
import { getBeanExchanges } from 'apiSdk/bean-exchanges';
import { BeanExchangeInterface } from 'interfaces/bean-exchange';
import { getEvents } from 'apiSdk/events';
import { EventInterface } from 'interfaces/event';
import { getTutorials } from 'apiSdk/tutorials';
import { TutorialInterface } from 'interfaces/tutorial';

import { TutorialListPage as ShowCaseList } from 'pages/tutorials';

function DashboardPage() {
  const { session } = useSession();

  const beanExchangeStat = useSWR<PaginatedInterface<BeanExchangeInterface>>(
    () => `/bean-exchanges`,
    () => getBeanExchanges({ limit: 1, offset: 0 }),
  );
  ...
  const tutorialStat = useSWR<PaginatedInterface<TutorialInterface>>(
    () => `/tutorials`,
    () => getTutorials({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Bean Exchanges',
      result: beanExchangeStat?.data?.totalCount,
      isLoading: beanExchangeStat.isLoading,
    ...
    {
      title: 'Tutorials',
      result: tutorialStat?.data?.totalCount,
      isLoading: tutorialStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
