...
import { getTutorialById } from 'apiSdk/tutorials';
import { TutorialInterface } from 'interfaces/tutorial';

function TutorialViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<TutorialInterface>(
    () => (id ? `/tutorials/${id}` : null),
    () =>
      getTutorialById(id, {
        relations: ['user'],
      }),
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('tutorial', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/tutorials/edit/${id}`} passHref legacyBehavior>
                    <Button
